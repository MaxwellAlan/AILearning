...
class CustomedDataSet(Dataset):
    def __init__(self, train=True):
        self.train = train
        if self.train:
            trainX = pd.read_csv('data/4-Kaggle/digit-recognizer/input/train.csv')
            trainY = trainX.label.as_matrix().tolist()
            trainX = trainX.drop('label', axis=1).as_matrix().reshape(trainX.shape[0], 1, 28, 28)
            self.datalist = trainX
            self.labellist = trainY
        else:
            testX = pd.read_csv('data/4-Kaggle/digit-recognizer/input/test.csv')
...
saveResult(testLabel, 'data/4-Kaggle/digit-recognizer/ouput/Result_pytorch_CNN.csv')
...
# aa.to_csv('data/4-Kaggle/digit-recognizer/ouput/submit_pytorch.csv', index=False)
