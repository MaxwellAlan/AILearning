...
def opencsv():
    print('Load Data...')
    # 使用 pandas 打开
    dataTrain = pd.read_csv('data/4-Kaggle/digit-recognizer/input/train.csv')
    dataTest = pd.read_csv('data/4-Kaggle/digit-recognizer/input/test.csv')
...
def trainDRSVM():
    ...
    storeModel(optimalSVMClf, 'data/4-Kaggle/digit-recognizer/ouput/Result_sklearn_SVM.model')
    storeModel(pcaPreData, 'data/4-Kaggle/digit-recognizer/ouput/Result_sklearn_SVM.pcaPreData')
...
def preDRSVM():
    ...
    # 加载模型和数据
    optimalSVMClf = getModel('data/4-Kaggle/digit-recognizer/ouput/Result_sklearn_SVM.model')
    pcaPreData = getModel('data/4-Kaggle/digit-recognizer/ouput/Result_sklearn_SVM.pcaPreData')

    # 结果预测
    testLabel = optimalSVMClf.predict(pcaPreData)

    # 结果的输出
    saveResult(testLabel, 'data/4-Kaggle/digit-recognizer/ouput/Result_sklearn_SVM.csv')
    print("finish!")
    stopTime = time.time()
